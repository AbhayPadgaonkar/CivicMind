...
export default function InputPage() {
  ...
  const handleFetchComplaints = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        setLogs((prev) => [
          ...prev,
          "> Authentication required. Redirecting to Secure Auth...",
        ]);
        setTimeout(() => {
  ...
  const uploadFiles = async (files: FileList) => {
    ...
    setLogs((prev) => [
      ...prev,
      `> Uploading ${files.length} manual document(s)...`,
    ]);
    ...
    try {
      ...
      setLogs((prev) => [
        ...prev,
        `> Success: Processed ${response.data.results.length} files.`,
      ]);

      // ---------------------------------------------------------
      // KEY CHANGE: TRANSFORM DATA FOR DASHBOARD COMPATIBILITY
      // ---------------------------------------------------------
      const newEntries = response.data.results.map(
        (item: any, index: number) => ({
          // Fields for THIS Page (Input Page Table)
          id: `manual-${Date.now()}-${index}`,
          source: item.extracted.subject || item.filename,
          sender: item.extracted.sender || "Manual Upload",
          contentType: item.filename.endsWith(".pdf")
            ? "PDF Report"
            : "Document",
  ...
  const handleProcessEmails = async () => {
    ...
    setLogs((prev) => [...prev, "> Initiating AI Analysis Pipeline..."]);
    ...
    localStorage.setItem("dashboardData", JSON.stringify(data.results));
    router.push("/dashboard/result_page");
  };
      ...
      <main className="pt-28 pb-12 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            ...
            <p className="text-gray-400">
              Connect external sources to feed the Decision Support System.
            </p>
          </div>
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Source: Reports & Grievances (Gmail)
                    </p>
                  </div>
                </div>

                <p className="text-gray-400 mb-8 max-w-lg leading-relaxed">
                  Fetch unread emails, recursively extract PDF/CSV attachments,
                  and categorize grievances automatically.
                </p>
            ...
            <div className="h-full min-h-[500px] max-h-[600px] bg-[#0A0510] rounded-3xl border border-white/10 p-6 relative overflow-hidden flex flex-col">
              ...
              <div className="flex-1 text-xs space-y-3 overflow-y-auto font-mono">
                <div className="text-gray-500">
                  Last login: {loginTime || "..."} on ttys002
                  <br />
        ...
        <div className="mt-12">
                ...
                {fetchedEmails.length === 0 ? (
                  ...
                  fetchedEmails.map((email, idx) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <span className="text-yellow-400 flex items-center gap-2 text-xs font-bold uppercase tracking-wide">
          ...
          {fetchedEmails.length > 0 && (
            ...
            >
              {/* Make sure this Link points to your actual Dashboard page location */}

              <button
                onClick={handleProcessEmails}
                className="group relative px-8 py-4 bg-white text-black font-bold rounded-xl hover:scale-[1.02] transition-transform shadow-[0_0_40px_rgba(255,255,255,0.3)]"
              >
                ...
                <span className="relative z-10 flex items-center gap-3">
                  Proceed to AI Analysis <ArrowRight className="w-5 h-5" />
                </span>
              </button>
            </motion.div>
          )}
