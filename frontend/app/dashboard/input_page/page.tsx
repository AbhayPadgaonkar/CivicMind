...
export default function InputPage() {
  ...
  const handleFetchComplaints = async () => {
    ...
    try {
      ...
      setLogs(prev => [...prev, "> Error: Connection failed. Check network."]);
      setIsFetching(false);
    }
  };
  ...
  const uploadFiles = async (files: FileList) => {
    ...
    try {
      ...
      const newEntries = response.data.results.map((item: any, index: number) => ({
        ...
        contentType: item.filename.endsWith('.pdf') ? "PDF Report" : "Document",
      ...
      setLogs(prev => [...prev, "> Error: Upload failed. Is the backend running?"]);
    } finally {
        ...
        <div className="mt-12">
           ...
           {fetchedEmails.length > 0 && (
              ...
              >
                ...
                <Link href="/dashboard/result_page">
